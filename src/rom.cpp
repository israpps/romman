...
rom::~rom() {
    ...
    for (size_t x = 0; x < files.size(); x++) {
        if (files[x].ExtInfoData != nullptr) {
            FREE(files[x].ExtInfoData);
        }
        if (files[x].FileData != nullptr) {
            FREE(files[x].FileData);
        }
    }
    files.clear();
}
    ...
    if ((F = fopen(path.c_str(), "rb")) != NULL) {
        ...
        if (strcmp(Fname, "RESET")) {
            if (rom::fileExists(Fname) != RET_OK) {
                ...
                if (isFixed)
                    AddExtInfoStat(&file, EXTINFO_FIELD_TYPE_FIXED, nullptr, 0);
            ...
            if (size > BOOTSTRAP_MAX_SIZE) {
                DERROR(
                    "# ERROR: requested file exceeds max size for bootstrap program\n"
                    "\tMax size: 0x%x\n\tFile size %x\n",
...
#define FIX_ALIGN(x)                                     \
    if ((FileAlignMargin = (ftell(OutputFile) % x)) > 0) \
        fseek(OutputFile, x - FileAlignMargin, SEEK_CUR);
